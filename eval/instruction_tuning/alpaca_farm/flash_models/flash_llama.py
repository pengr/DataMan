...
import einops
import torch
from flash_attn.flash_attn_interface import flash_attn_varlen_qkvpacked_func  # <代码修改>，原来是：flash_attn_unpadded_qkvpacked_func
...
class LlamaAttention(modeling_llama.LlamaAttention):
    ...
    def forward(  # noqa
        ...
        if past_key_value is None:
            ...
            attn_output = flash_attn_varlen_qkvpacked_func(  # <修改代码>
                qkv=qkv,
...
class LlamaDecoderLayer(modeling_llama.LlamaDecoderLayer):
    def __init__(self, config: modeling_llama.LlamaConfig, layer_idx: int):  # <fix>, add layer_idx
        super().__init__(config=config, layer_idx=layer_idx)  # <fix>, add layer_idx
        del self.self_attn

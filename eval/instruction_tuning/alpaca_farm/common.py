...
def safe_save_model_for_hf_trainer(
    ...
    # if trainer.fsdp is not None: # <修改代码>
    if trainer.is_fsdp_enabled is not False:
        ...
        cfg = FullStateDictConfig(offload_to_cpu=False, rank0_only=rank0_only)
...
def get_transformer_hidden_size(model: transformers.PreTrainedModel):
    ...
    elif isinstance(model, transformers.T5ForConditionalGeneration) or isinstance(model, transformers.T5EncoderModel):

...
class DataCollator:
    ...
    def __call__(self, features):
        # input_ids_ = [torch.tensor(item["input_ids"], dtype=torch.long) for item in features]  # <fix>
        # labels_ = [torch.tensor(item["input_ids"], dtype=torch.long) for item in features]      # <fix>
        input_ids_ = [torch.tensor(json.loads(item["input_ids"]), dtype=torch.long) for item in features]   # <fix>, "input_ids"对应的是string类型的列表
        labels_ = [torch.tensor(json.loads(item["input_ids"]), dtype=torch.long) for item in features]      # <fix>, "input_ids"对应的是string类型的列表
...
def main():
    ...
    # 修改代码,因为公司网络访问不到huggingface,所以无法通过args.config_name来加载模型，只能通过args.model_name_or_path，
    # 但会使得原本代码以为是用从Sheard-LLaMA-1.3B模型中加载模型参数做CPT，故注释掉这部分代码，让LLM从头开始预训练

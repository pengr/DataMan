...
from transformers import (
    ...
    HfArgumentParser,
    set_seed,
    AutoModelForCausalLM, # <fix>,要支持Qwen2.5-1.5B模型的预训练
)
...
def main():
    ...
    # <fix>, 要同时支持Sheared-LLAMA-1.3B, Qwen2.5-1.5B模型的预训练
    if args.model_name_or_path:
        ...
        logger.warning(f"Initializing new AutoModelForCausalLM from scratch")
    ...
    if training_args.do_train:
        checkpoint = None
        if training_args.resume_from_checkpoint is not None:  # 手动指定断点续训的检查点文件
            checkpoint = training_args.resume_from_checkpoint
        elif last_checkpoint is not None:                     # 自动加载output_dir最后一个checkpoint进行断点续训

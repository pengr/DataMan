...
class TokenizeMapper:
    def __init__(self, args, tokenizer):  # <fix>
        self.args = args
        self.tokenizer = tokenizer          # <fix>
    ...
    def __call__(self, examples, indices=None):
        ...
        if not self.args.remove_columns:
            # output.update({field: [] for field in examples})              # <修改代码>
            output.update({field if field != 'meta' else 'source_domain': [] for field in examples})
                ...
                if not self.args.remove_columns:
                    ...
                    for field in examples:
                        if field in additional_fields:
                            continue
                        if field == "meta":      # <修改代码>
                            source_domain = examples[field][i]["redpajama_set_name"].replace("RedPajama", "")
                            output["source_domain"].append(source_domain)
                            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_workers", type=int, default=None, help="Workers.")
    parser.add_argument("--remove_columns", action="store_true", help="Remove columns.")

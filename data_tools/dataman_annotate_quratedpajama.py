...
[14]Overall Score:_/5""",
        '_only': """Please give an overall score for the text:
Text: {text}
Overall Score:_/5""",
    ...
    try:
        ...
        with open(input_path, 'r', encoding='utf-8', errors='ignore') as rf:
            ...
            for lineID, line in tqdm(enumerate(rf, start_position), total=end_position-start_position, desc=f"Worker {worker_id} Processing {start_position}-{end_position}"):
                ...
                elif data_format == "quratedpajama":
                    data = json.loads(line.strip())
                    text, writing_style_average, facts_and_trivia_average, educational_value_average, required_expertise_average, document_index, source_domain, ppl = \
                        data.get('text'), data.get('writing_style_average'), data.get('facts_and_trivia_average'), data.get('educational_value_average'), data.get('required_expertise_average'), \
                        data.get('document_index'), data.get('source_domain'), None
                ...
                result = {"text": text, "writing_style_average": writing_style_average, "facts_and_trivia_average": facts_and_trivia_average, \
                    "educational_value_average": educational_value_average, "required_expertise_average": required_expertise_average, \
                        "document_index": document_index, "source_domain": source_domain, "length": f"{text_len}"}
                if ppl is not None:
                    result["ppl"] = "%.2f" % ppl
    ...
    try:
        ...
        with open(input_path, 'r', encoding='utf-8') as rf:
            ...
            for lineID in tqdm(range(start_position, end_position, batch_size), total=((end_position-start_position)//batch_size)+1, desc=f"Worker {worker_id} Inferencing {start_position}-{end_position}"):
                chat_texts, texts, document_indexes, source_domains, text_lens, ppls = [], [], [], [], [], []
                writing_style_averages, facts_and_trivia_averages, educational_value_averages, required_expertise_averages = [], [], [], []
                for _ in range(batch_size):
                    ...
                    document_indexes.append(data['document_index'])
                    source_domains.append(data['source_domain'])
                    text_lens.append(data['length'])
                    if data_format == "ppl":
                        ppls.append(data['ppl'])
                    writing_style_averages.append(data['writing_style_average'])
                    facts_and_trivia_averages.append(data['facts_and_trivia_average'])
                    educational_value_averages.append(data['educational_value_average'])
                    required_expertise_averages.append(data['required_expertise_average'])
                ...
                for i, output in enumerate(outputs):
                    ...
                    result_dict.update({
                        ...
                        "document_index": document_indexes[i],
                        "writing_style_average":writing_style_averages[i],
                        "facts_and_trivia_average":facts_and_trivia_averages[i],
                        "educational_value_average":educational_value_averages[i],
                        "required_expertise_average":required_expertise_averages[i],
                    })

                    # 根据 model_type 和 output_text 来设置 json 字段
                    if model_type.endswith('score_only'):
                        result_dict["overall_score"] = output_text
                    elif model_type.endswith('domain_only'):
...
def multiprocess_preprocess(args, input_path, output_path):
    ...
    def count_lines(filename):
        result = subprocess.run(['python', 'utils/num_of_lines.py',
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_format', default="qwen", type=str, help='data format', choices=['ppl', 'qwen', 'slimpajama', 'commoncrawl', 'quratedpajama'])

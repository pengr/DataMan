...
class HuggingFaceDatasetReader(BaseReader):
    ...
    def run(self, data: DocumentsPipeline = None, rank: int = 0, world_size: int = 1) -> DocumentsPipeline:
        ...
        from datasets import load_from_disk, concatenate_datasets # <fix> 

        if data:
            yield from data
        if self.dataset == "arrow": # <fix> 
            ds = concatenate_datasets([load_from_disk(path) for path in self.dataset_options["data_files"]])
        else:

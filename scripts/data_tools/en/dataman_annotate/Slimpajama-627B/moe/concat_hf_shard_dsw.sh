...
for FILE in "${FILES[@]}"; do
    input_files=""
    for shard in {0..1}; do
        input_files+="$DATA_DIR/DataMan/Slimpajama-627B/annotated/all_rating/moe/${FILE}_annotated/${shard} "
    done

    python $CODE_DIR/data_tools/concat_hf_shard.py \
    --inputs $input_files \
    --output $DATA_DIR/DataMan/Slimpajama-627B/annotated/all_rating/moe/${FILE}_annotated \
    --num_workers 64
done
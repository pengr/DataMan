...
for chunk in {1..10}; do
    for half in {1..2}; do
        input_files+="$DATA_DIR/DataMan/Slimpajama-627B/annotated/fineweb_top3step/train_chunk${chunk}_half${half}/huggingface "
    done
done
...
--inputs $input_files \
--output $DATA_DIR/DataMan/Slimpajama-627B/data_filtering/uniform/train_filtered_fineweb \
--metric_field overall_score \
--seq_len_field length \
--domain_field source_domain application_domain \
--tokens 30_000_000_000 \
--tokens_per_shard 1_500_000_000 \
--margin 0.1 \
--seed 42 \
--num_workers 128 \

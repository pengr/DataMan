...
for chunk in {0..17}; do
    input_files+="$DATA_DIR/DataMan/princeton-nlp/QuRatedPajama-260B/data_filtering/educational_value-sample_with_temperature2.0/train_filtered/${chunk}-18 "
done
dataset=${DATASET:-"$input_files"}
# suffix to append to run name
suffix=${SUFFIX:-"educational_value-sample_with_temperature2.0"}
# name of training session
run_name="lm-1.3B-select_60B_tokens_by-${suffix}"
out_dir="$CKPT_DIR/DataMan/pretrain/princeton-nlp/$run_name"
mkdir -p $out_dir
...
base_arguments=(
    ...
    --do_train                           # 设置进行训练过程
    --model_name_or_path "$CKPT_DIR/base/Sheared-LLaMA-1.3B" # <修改代码>，设置模型路径
    # --resume_from_checkpoint "$CKPT_DIR/DataMan/princeton-nlp/lm-1.3B-select_60B_tokens_by-educational_value-sample_with_temperature2.0"
    # --config_name $arch                # <修改代码>，注释掉arch变量指定的模型配置名称，因为无法访问hf下载
    --config_overrides ""                # 没有配置覆盖
    # --tokenizer_name $arch             # <修改代码>，注释掉tokenizer变量，因为无法访问hf下载
    --tokenizer_name "$CKPT_DIR/base/Sheared-LLaMA-1.3B"    # <修改代码>，设置tokenizer的路径
...
torchrun $DISTRIBUTED_ARGS $CODE_DIR/pretrain/train_language_model_qurating.py \

...
dataset=${DATASET:-"$DATA_DIR/DataMan/Slimpajama-627B/data_filtering/medicine/train_filtered/0"}
# suffix to append to run name
suffix=${SUFFIX:-"uniform-sample_with_overall_score5.0_medicine_cpt"}
# name of training session
run_name="lm-$(basename $arch)-select_30B_tokens_by-${suffix}"
out_dir="$CKPT_DIR/DataMan/pretrain/en/$run_name"
mkdir -p $out_dir
...
base_arguments=(
    ...
    --do_train                           # 设置进行训练过程
    --model_name_or_path "$CKPT_DIR/DataMan/pretrain/en/lm-Sheared-LLaMA-1.3B-select_30B_tokens_by-uniform-sample_with_overall_score5.0/checkpoint-merge-69000-70000-71000"  # <修改代码>，设置模型路径
    # --model_name_or_path "$CKPT_DIR/base/Sheared-LLaMA-1.3B" # <修改代码>，设置模型路径
    # --resume_from_checkpoint "$CKPT_DIR/DataMan/pretrain/en/lm-Sheared-LLaMA-1.3B-select_30B_tokens_by-uniform-sample_with_overall_score5.0"

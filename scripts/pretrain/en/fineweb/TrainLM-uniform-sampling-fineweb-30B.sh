...
for chunk in {0..17}; do
    input_files+="$DATA_DIR/DataMan/Slimpajama-627B/data_filtering/uniform/train_filtered_fineweb/${chunk} "
done
dataset=${DATASET:-"$input_files"}
# suffix to append to run name
suffix=${SUFFIX:-"uniform-sampling-fineweb"}
# name of training session
run_name="lm-Sheared-LLaMA-1.3B-select_30B_tokens_by-${suffix}"
...
base_arguments=(
    ...
    # --resume_from_checkpoint "$CKPT_DIR/DataMan/pretrain/en/lm-1.3B-select_30B_tokens_by-uniform-sampling-fineweb"
